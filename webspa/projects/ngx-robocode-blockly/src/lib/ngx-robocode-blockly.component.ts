...
export class NgxRobocodeBlocklyComponent implements OnInit, AfterViewInit {
  ...
  @Output() public code: EventEmitter<string> = new EventEmitter<string>();
  ...
  workspaceToCode(workspaceId: string) {
    const codelabGenerator = new Blockly.Generator('JSON');
    const code = ((Blockly as any)[CODE_LANGUAGE].workspaceToCode(this.workspace)) as string;
    this.code.emit(code);
  }
}
